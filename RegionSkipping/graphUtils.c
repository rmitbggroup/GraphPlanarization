...
int  _InitGraph(graphP theGraph, int N)
{
     ...
     int a=((theGraph->V = (vertexRecP) calloc(Vsize, sizeof(vertexRec)))==NULL);

     int b=((theGraph->VI = (vertexInfoP) calloc(VIsize, sizeof(vertexInfo))) == NULL);


     //printf("size: %lu\n", Esize * sizeof(edgeRec));
     int c=((theGraph->E = (edgeRecP) calloc(Esize, sizeof(edgeRec))) == NULL);


     int d=((theGraph->BicompRootLists = LCNew(VIsize)) == NULL);

     int e=((theGraph->sortedDFSChildLists = LCNew(VIsize)) == NULL);

     int f=((theGraph->theStack = sp_New(stackSize)) == NULL);

     int g=( (theGraph->extFace = (extFaceLinkRecP) calloc(Vsize, sizeof(extFaceLinkRec))) == NULL);


     int h=((theGraph->edgeHoles = sp_New(Esize / 2)) == NULL);
