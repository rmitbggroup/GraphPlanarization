...
// store sourceVirtualPoint and subtreePNVertices. W is the current pNVertex and c is the current vertex.
/*
void storeInfo(graphP theGraph, int W, int c, int v){
	 int pNV=theGraph->VI[c].dominatingVertex[v];
	 int virPoint=theGraph->VI[pNV].sourceVirtualPoint;
	 theGraph->VI[W].sourceVirtualPoint=virPoint;
	 theGraph->VI[virPoint].pNVCount=theGraph->VI[virPoint].pNVCount+1;
	 int index=theGraph->VI[virPoint].pNVCount;
	 theGraph->VI[virPoint].subtreePNVertices[index-1]=W;
	// printf("the dominatingvertex is %d, virtualPoint is %d, index is %d \n",pNV,virPoint,index);
}
*/
/*
void newWalkUp(graphP theGraph, int v, int e)
{
int potentialNV=gp_GetNeighbor(theGraph, e);
//printf("processing vertex %d \n",v);
//printf("!!potentialNV is %d \n",potentialNV);
if(theGraph->VI[potentialNV].noisiness==1)
	return;
int  W = potentialNV;
     ...
     while (Zig != v)
     {
         ...
         if (gp_IsVirtualVertex(theGraph, (nextZig = gp_GetExtFaceVertex(theGraph, Zig, 1^ZigPrevLink))))
         {
				 ...
				 if (gp_GetVertexVisitedInfo(theGraph, Zig) == v){
					 storeInfo(theGraph,potentialNV,Zig,v);
					 break;
				 }
				 ...
				 if (gp_GetVertexVisitedInfo(theGraph, nextZag) == v) {
					 storeInfo(theGraph,potentialNV,nextZag,v);
					 break;
				 }
         ...
         else if (gp_IsVirtualVertex(theGraph, (nextZag = gp_GetExtFaceVertex(theGraph, Zag, 1^ZagPrevLink))))
         {
				 if (gp_GetVertexVisitedInfo(theGraph, Zag) == v) {
					 storeInfo(theGraph,potentialNV,Zag,v);
					 break;
				 }
				 R = nextZag;
				 ...
				 if (gp_GetVertexVisitedInfo(theGraph, nextZig) == v) {
					 storeInfo(theGraph,potentialNV,nextZig,v);
					 break;
				 }
         ...
         {
				 if (gp_GetVertexVisitedInfo(theGraph, Zig) == v) {
					 storeInfo(theGraph,W,Zig,v);
					 break;
				 }
				 if (gp_GetVertexVisitedInfo(theGraph, Zag) == v){
					 storeInfo(theGraph,W,Zag,v);
					 break;
				 }
				 R = NIL;
         ...
         theGraph->VI[Zig].dominatingVertex[v]=potentialNV;
         gp_SetVertexVisitedInfo(theGraph, Zag, v);
         theGraph->VI[Zag].dominatingVertex[v]=potentialNV;
         ...
         else
         {
             ...
             //if current root is v', append w to its subtreePNVertices.
             if(Zig==v){

            	 theGraph->VI[potentialNV].sourceVirtualPoint=R;

            	 theGraph->VI[R].pNVCount=(theGraph->VI[R].pNVCount)+1;
            	 int index=theGraph->VI[R].pNVCount;
            	 theGraph->VI[R].subtreePNVertices[index-1]=potentialNV;
...
*/
/*
void findNV(graphP theGraph,int RootVertex,struct nVList *list){

	for(int i=1;i<=theGraph->VI[RootVertex].pNVCount;i++){
		int DFIvertex=theGraph->VI[RootVertex].subtreePNVertices[i-1];
		int orgVertex=theGraph->VI[DFIvertex].originalIndex;
		if (gp_IsArc(gp_GetVertexPertinentEdge(theGraph, DFIvertex))){
			list->count++;
			int count=list->count;
			list->nV[count-1]=orgVertex;
			gp_SetVertexPertinentEdge(theGraph,DFIvertex,NIL);
			theGraph->VI[DFIvertex].noisiness=1;
		}
	}

}
*/
int newWalkUp(graphP theGraph, int v, int e, int pertinentVertex[],int *pos)
{
...
int  nextZig, nextZag, R;
     ...
     while (Zig != v)
     {
         ...
         else
         {
			 ...
			 if (gp_GetVertexLowpoint(theGraph, gp_GetDFSChildFromRoot(theGraph, R)) < v){
				  gp_AppendVertexPertinentRoot(theGraph, Zig, R);
				  pertinentVertex[*pos]=Zig;
				  *pos=*pos+1;
			 }
			 else{
				 pertinentVertex[*pos]=Zag;
				 *pos=*pos+1;
...
int  newWalkDown(graphP theGraph, int v, int RootVertex)
{
	...
	int mark=0;
	     ...
	     for (RootSide = 0; RootSide < 2; RootSide++)
	     {
	         ...
	         while (W != RootVertex)
	         {
	             ...
	             if (gp_IsArc(gp_GetVertexPertinentEdge(theGraph, W))&&(theGraph->V[W].noisiness!=1))
	             {
	             ...
	             else
	             {
	                 ...
	                 {
	                	 	 	 // ascend to the parent component since the stopping vertex is on a non-root component.
	                	 	 	 if(sp_NonEmpty(theGraph->theStack)){
	                	 	 		 mark=1;

	                	 	 		sp_Pop2(theGraph->theStack, W, WPrevLink);
	                	 	 		sp_Pop2(theGraph->theStack, W, WPrevLink);
	                	 	 		int root=gp_GetVertexFirstPertinentRoot(theGraph, W);

	                	 	 		if (gp_IsVertex(root)){
	                	 	 				gp_DeleteVertexPertinentRoot(theGraph, W, root);
	                	 	 		}
	                	 	 		...
	                	 	 		int neighbor1=gp_GetExtFaceVertex(theGraph, RootVertex, 1^RootSide);
	                	 	 		int neighbor2=gp_GetExtFaceVertex(theGraph, RootVertex, RootSide);

	                	 	 		 if((neighbor1==neighbor2)&&(neighbor1==W)){
								 ...
								 // Terminate the Walkdown traversal since it encountered the stopping vertex on the root component.
								 break;
	                	 	 	 }
	                 }
...
int optimizedPlanarization(graphP theGraph, int embedFlags)
{
	...
	int *pertinentVertex=(int*)malloc( (theGraph->N)*sizeof(int) );

		  for(int i=0;i<theGraph->N;i++)
			  pertinentVertex[i]=0;
	    ...
	    // In reverse DFI order, embed the back edges from each vertex to its DFS descendants.
	    for (v = gp_GetLastVertex(theGraph); v>1; v--)
	    {
	    		  int count=0;
	    			int pos=0;
	          RetVal = OK;
	          ...
	          while (gp_IsArc(e))
	          {
					  ...
					  if(theGraph->V[W].noisiness==1){
						  e = gp_GetNextArc(theGraph, e);
						  if (e == gp_GetVertexFwdArcList(theGraph, v))
							  e = NIL;
						  continue;
					  }
					  //printf("pertinent vertex is %d \n",W);



					  count++;

					  newWalkUp(theGraph, v, e,pertinentVertex,&pos);
					  //newWalkUp(theGraph, v, e,pertinentVertex);
					  theGraph->pertinentList[count-1]=W;
					  e = gp_GetNextArc(theGraph, e);
					  if (e == gp_GetVertexFwdArcList(theGraph, v))
						  e = NIL;
	          }
	          ...
	          while (gp_IsVertex(c))
	          {
					  if (gp_IsVertex(gp_GetVertexPertinentRootsList(theGraph, c)))
					  {
						  RetVal = newWalkDown(theGraph, v, gp_GetRootFromDFSChild(theGraph, c));
	          ...
	          //Clear all pertinent roots
	          int i=0;
	           	  	  while(i!=pos){
	           	  		  int W=pertinentVertex[i];
	           	  		  gp_SetVertexPertinentRootsList(theGraph, W, NIL);
	           	  		  i++;
	           	  	  }
	          ...
	          for(int i=1;i<=count;i++){
	        	  	  int vertex=theGraph->pertinentList[i-1];
	        	  	  if(gp_GetVertexPertinentEdge(theGraph, vertex)!=0){
	        	  		  mark=1;
	        	  		  gp_SetVertexPertinentEdge(theGraph, vertex, NIL);
	        	  	  }
	          }
	          if(mark==1){
				  theGraph->V[v].noisiness=1;
				  int pos=theGraph->V[v].originalIndex;
				  theGraph->obstructList[pos]=1;
				  theGraph->obstructCount++;
				  if(pos<theGraph->minObstruct){
					theGraph->minObstruct=pos;
				  }
	          }
